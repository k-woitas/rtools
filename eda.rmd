...
There are `r ncol(sw)` variables, of which 3 are a list type. The other variables are numbers or characters, last ones partly better to be transformed into categorial (i.e. factor or dummy) variables.  
...
```{r}
# skimming a character, numeric, integer, factor variable
...
```{r}
sapply(sw[,c(4:6,8:11)], nlevels)    # number of levels for the categorials (columns 4 to 6 and 8 to 11)
sapply(sw[,2:3], median, na.rm=T)    # median score for the numerics (columns 2 and 3); "na.rm" for ignoring the NA's
```
...
For numeric variables there are dedicated statistical commands like:
```{r}
...
Turning numeric values into standard normal values can be done in a quite a simple way using the `Make.Z` command, or alternatively with `scale`. Both can be also applied on whole dataframes.
```{r}
...
The `{dplyr}` function `group_by` easily splits the dataframe into groups along certain variables (factors and numeric!) and works fine with the pipe.
```{r}
...
And this works for grouping along continous variables, too  -- and of course with specifying the statistical values.
```{r}
